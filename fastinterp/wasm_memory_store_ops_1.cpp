...
// the case that memory offset has been spilled
//
struct FIMemoryStoreOpsSpilledImpl
{
    ...
    static void f(uintptr_t stackframe, OpaqueParams... opaqueParams, [[maybe_unused]] SrcType qa1) noexcept
    {
        ...
        if constexpr(!isInRegister)
        {
            ...
            {
                var_offset = *internal::GetStackTop<uint32_t>(stackframe);
                operand = *internal::GetStackTop<SrcType>(stackframe);
            }
            else
            {
                var_offset = *internal::GetStack2ndTop<uint32_t>(stackframe);
                operand = *internal::GetStackTop<SrcType>(stackframe);
            }
        }
        else
...
void __pochivm_build_fast_interp_library__()
{
    ...
    RegisterBoilerplate<FIMemoryStoreOpsSpilledImpl>();
}
